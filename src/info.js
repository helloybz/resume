        ...
        "items": [
            {
                ...
                보유한 오디오 데이터는 시간-세기 축을 갖는 waveform 데이터 입니다. waveform을 그대로 활용할 수도 있지만, 보다 더 풍부한 feature를 활용하기 위해 시간-주파수-세기 축을 갖는 Spectrogram으로 변형하여 활용했습니다. JSON 기반의 노트 패턴 데이터는 학습 설정에 맞는 Tensor form으로 변환할 수 있도록 번역 스크립트를 작성해 유용하게 사용했습니다.
                처음에는 자연어 번역 문제에 대입하여 NLP분야에서 주로 쓰이는 RNN, Transformer와 같은 시퀀스 모델을 적용했습니다. 결론부터 말하자면 여러가지 문제점이 발견되어 모델 구조를 변경했습니다. 첫째로 입력 가능한 오디오의 길이가 10초 남짓으로 짧아서 생성된 노트 패턴들이 서로 자연스럽게 이어지지 못하는 문제가 있었습니다. 둘째로, 1/32박에 해당하는 길이의 오디오를 word와 같은 token의 역할로 활용했기 때문에 박자 계산의 위해서 곡의 빠르기 정보에 대한 사전 정보가 필요했습니다. 시퀀스 모델 대신 Fully Convolution 모델로 변경하여 곡의 빠르기를 고려할 필요가 없어졌고, 모델이 요구하는 메모리가 적어 곡 전체를 입력할 수 있었습니다.
                ...
                해당 프로젝트가 실제로 사용될 수는 없었지만, 연구 목적의 프로젝트와 달리 새로운 경험을 할 수 있었습니다.`
            },
            {
            ...
            },
            {
                ...
                "at": "SnapmeKorea",
                "description": `
                사진작가와 관광객을 매칭시켜주는 O2O 서비스인 스냅미코리아의 홈페이지를 제작을 의뢰받았습니다. 학부 재학중 웹개발을 익히면서 친구와 같이 맡은 프로젝트이며 저는 백엔드 개발을 맡았습니다. Django + PostgreSQL을 사용했으며, cafe24에서 호스팅하였습니다.
                `,
            }
        ]
    },
